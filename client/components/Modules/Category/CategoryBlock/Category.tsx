...
import { CategoryCard } from '@/components/Modules/Category/CategoryCard/CategoryCard';
    ...
    return (<>
        {category.length > 0 ? <div className={cl.item_block}>
            {category.map((item: any) => {
                return <CategoryCard id={item.id} isAdmin={user?.user_role === 'admin' ? true : false} img={item.imageSrc} title={item.category_name} description={item.description} key={item.id} link={`/category/${item.id}`} />
            })} </div>
        ...
        {user?.user_role === 'admin' ? <div className={cl.addcategory}>
            ...
            <Modal visible={visible} setVisible={setVisible}>
                <form onSubmit={(e) => { e.preventDefault(); addCategory(); }}>
                    ...
                    <label>Картинка категории</label><br />
